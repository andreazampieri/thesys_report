...
\begin{document}
	...
	\section{Reproduction of the SoA Tecnique}
		...
		\subsection{Node Embedding}
			...
			\end{itemize}
			The algorithm used is \textit{Word2Vec}; it's based on a Skip-gram model, i.e. the training samples are pairs of words that are in the same context window even if they aren't adjacent. (E.g. using the previous sample phrase, the pairs generated from the word fox, would be (quick,fox),(brown,fox),(fox,jumps),(fox,over)).\medskip\\
			...
			Note that a 100-dimensional-space-representation for a node is far more compressed than it's adjacency list.
